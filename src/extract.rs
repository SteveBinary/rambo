...
pub fn extract_creation_datetime_from_media_source(
    media_source: MediaSource<File>,
    media_parser: &mut MediaParser,
) -> anyhow::Result<DateTime<FixedOffset>> {
    if media_source.has_exif() {
        let exif_iter: ExifIter = media_parser
            .parse(media_source)
        ...
        Err(anyhow::anyhow!(
            "The media source has no EXIF or track data!"
        ))
    }
}
...
fn extract_creation_datetime_from_exif(exif: &Exif) -> anyhow::Result<DateTime<FixedOffset>> {
    ...
    Err(anyhow::anyhow!(
        "Could not get the creation datetime from EXIF data!"
    ))
}

const TRACK_INFO_TAGS_FOR_CREATION_DATETIME: [TrackInfoTag; 1] = [TrackInfoTag::CreateDate];

fn extract_creation_datetime_from_track_info(
    ...
    Err(anyhow::anyhow!(
        "Could not get the creation datetime from track info data!"
    ))
}
